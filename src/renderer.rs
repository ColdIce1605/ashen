...
impl Renderer {
    pub fn new(window: &Window, screen_resolution: vk::Extent2D) -> Self {
        ...
        let (depth_image, depth_image_view, depth_image_memory) =
            create_depth_resources(&device, &swapchain_extent, &device_memory_properties);
    ...
    pub fn recreate_swapchain(&mut self) {
        ...
        self.present_image_views =
            create_image_views(&self.device, &present_images, self.surface_format);
...
mod tests {
    ...
    #[test]
    fn test_drop_renderer() {
        let renderer = Renderer::new(1920, 1080);
        assert_eq!(Ok(renderer), renderer);

        Renderer::drop(renderer);
        assert_eq!(2 + 2, 4);
    }
}
*/
